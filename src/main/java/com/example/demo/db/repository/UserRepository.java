...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    void updateIsDeletedAndUpdatedAtByUuid(Boolean isDeleted, Date updatedAt, String uuid);

    @Transactional
    @Modifying
    @Query("update UserEntity u set u.isDeleted = ?1, u.deletedAt = ?2, u.status = false where u.uuid = ?3 and u.status = true")
    void updateIsDeletedAndDeletedAtByUuidAndStatusTrue(Boolean isDeleted, Date deletedAt, String uuid);
